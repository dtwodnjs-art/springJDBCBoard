...
public class BoardDAO2 {
	...
	public int insertBoard(Board2 board) {
		...
		int count = jdbcTemplate.update(query, board.getUserId(), board.getUserpassword(), board.getTitle(),
				board.getContent());
	...
	public int updateBoard(Board2 board) {
		...
		int count = jdbcTemplate.update(query, board.getUserId(), board.getTitle(), board.getContent(), board.getNo());
		return count;
	}
	...
	public List<Board2> boardSearch(Board2 board) {
		...
		String query = "select * from jdbcBoard2 where " + searchItem + " like '%" + board.getKeyword()
				+ "%' order by no desc";
